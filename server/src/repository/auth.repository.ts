...
export class AuthRepository {
  static async create(data: CreationAttributes<User>, transaction?: Transaction): Promise<User> {
  ...
  static async findByEmail(email: string, transaction?: Transaction): Promise<User | null> {
    return User.findOne({ where: { email }, transaction });
  }

  static async list(where: Record<string, any> = {}, options: FindOptions = {}): Promise<any[]> {
